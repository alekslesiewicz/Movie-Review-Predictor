...
// File Name:      a2.cpp
//
// Author:         Aleks Lesiewicz
// CS email:       aleks@cs.wisc.edu
//
...
/**
 * @brief Cleans the text file passed in and outputs it to new file
 *
...
void fillDictionary(std::ifstream &newInFile,
    ...
    while(getline(newInFile, line)){
        ...
        //vector to hold words of each line
...
void scoreReviews(std::ifstream &testFile,
	...
	//clean line to output with predicted score
	string cleanLine;

        //calculate predicted review
        for (int i = 0; i < reviewWords.size(); i++) {
	cleanLine += (reviewWords.at(i) + " ");
        ...
        ratingsFile << fixed << setprecision(2) << predicted << " " << cleanLine;
...
/**
 * @brief Removes single letter words from a vector of strings.
 *
